...
\input{table/dair_official_baselines}
...
Table~\ref{tab:dair-mst-tradeoff} and Fig.~\ref{fig:dair-mst-pareto} analyze whether MST reduces communication-state cost without invalidating prediction quality. The original ConvLSTM hidden and cell states require 16.0 MiB per agent. The DS64-FP16 and Bottleneck12 variants reduce the state to 0.25 MiB per agent, corresponding to a $64\times$ reduction, while achieving higher IoU and Dynamic IoU than the uncompressed ConvLSTM-state baseline. This indicates that the transmitted state can be compact while still preserving the temporal information needed for compensation.

\input{table/dair_sensitivity}
